 ...
 * <tr><td>id</td><td>0x0</td><td>0x8</td><td></td><td>byte[]</td><td>{@link syncVisitor#id(ByteBuffer, int[], IntBuffer)}</td></tr>
 * 
 * @see hideftvads.sync#Atom
 * @see hideftvads.sync#id
 * </table>
 */
public enum sync { 
Atom(0x8)	{{
...
,id;
    ...
    static void index
        ...
        for (sync sync_ : values()) {
            String hdr = sync_.name();
            System.err.println("hdr:pos " + hdr + ':' + stack.position());
            sync_.subIndex(src, register, stack);
        }
    }
