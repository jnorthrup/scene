 ...
 * <tr><td>len</td><td>0x4</td><td>0x0</td><td></td><td>int</td><td>{@link AtomVisitor#len(ByteBuffer, int[], IntBuffer)}</td></tr>
 * <tr><td>tag</td><td>0x4</td><td>0x4</td><td></td><td>int</td><td>{@link AtomVisitor#tag(ByteBuffer, int[], IntBuffer)}</td></tr>
 *
 * @see hideftvads.Atom#len
 * @see hideftvads.Atom#tag
 *      </table>
 */
public enum Atom {
    len(0x4), tag(0x4);
    ...
    Atom(int... dimensions) {
        Method method = null;
        try {
            method = Class.forName(getClass().getName() + "Visitor").getMethod(name(), ByteBuffer.class, int[].class, IntBuffer.class);
        } catch (Exception e) {
        }
    ...
    static void index
        ...
        for (Atom Atom_ : values()) {
            String hdr = Atom_.name();
            System.err.println("hdr:pos " + hdr + ':' + stack.position());
            Atom_.subIndex(src, register, stack);
        }
    }
