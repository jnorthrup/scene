 ...
 * <tr><td>Atoms</td><td>0x0</td><td>0x8</td><td></td><td>byte[]</td><td>{@link trakVisitor#Atoms(ByteBuffer, int[], IntBuffer)}</td></tr>
 *
 * @see hideftvads.trak#Atom
 * @see hideftvads.trak#Atoms
 *      </table>
 */
public enum trak {
    Atom(0x8) {{
    ...
    static void index
        ...
        for (trak trak_ : values()) {
            String hdr = trak_.name();
            System.err.println("hdr:pos " + hdr + ':' + stack.position());
            trak_.subIndex(src, register, stack);
        }
    }
