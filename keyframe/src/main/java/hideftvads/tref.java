 ...
 * <tr><td>Atoms</td><td>0x0</td><td>0x8</td><td></td><td>byte[]</td><td>{@link trefVisitor#Atoms(ByteBuffer, int[], IntBuffer)}</td></tr>
 *
 * @see hideftvads.tref#Atom
 * @see hideftvads.tref#Atoms
 *      </table>
 */
public enum tref {
    Atom(0x8) {{
    ...
    static void index
        ...
        for (tref tref_ : values()) {
            String hdr = tref_.name();
            System.err.println("hdr:pos " + hdr + ':' + stack.position());
            tref_.subIndex(src, register, stack);
        }
    }
