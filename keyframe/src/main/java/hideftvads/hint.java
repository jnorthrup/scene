 ...
 * <tr><td>id</td><td>0x0</td><td>0x8</td><td></td><td>byte[]</td><td>{@link hintVisitor#id(ByteBuffer, int[], IntBuffer)}</td></tr>
 *
 * @see hideftvads.hint#Atom
 * @see hideftvads.hint#id
 *      </table>
 */
public enum hint {
    Atom(0x8) {{
    ...
    static void index
        ...
        for (hint hint_ : values()) {
            String hdr = hint_.name();
            System.err.println("hdr:pos " + hdr + ':' + stack.position());
            hint_.subIndex(src, register, stack);
        }
    }
