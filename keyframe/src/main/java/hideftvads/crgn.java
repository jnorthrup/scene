 ...
 * <tr><td>regionalSize</td><td>0x2</td><td>0x8</td><td></td><td>short</td><td>{@link crgnVisitor#regionalSize(ByteBuffer, int[], IntBuffer)}</td></tr>
 * <tr><td>regionBoundaryBox</td><td>0x8</td><td>0xa</td><td></td><td>long</td><td>{@link crgnVisitor#regionBoundaryBox(ByteBuffer, int[], IntBuffer)}</td></tr>
 * <tr><td>clippingRegionData</td><td>0x0</td><td>0x12</td><td></td><td>byte[]</td><td>{@link crgnVisitor#clippingRegionData(ByteBuffer, int[], IntBuffer)}</td></tr>
 *
 * @see hideftvads.crgn#Atom
 * @see hideftvads.crgn#regionalSize
 * @see hideftvads.crgn#regionBoundaryBox
 * @see hideftvads.crgn#clippingRegionData
 *      </table>
 */
public enum crgn {
    Atom(0x8) {{
        ___subrecord___ = hideftvads.Atom.class;
    }}, regionalSize(0x2), regionBoundaryBox(0x8), clippingRegionData;
    /**
    ...
    static void index
        ...
        for (crgn crgn_ : values()) {
            String hdr = crgn_.name();
            System.err.println("hdr:pos " + hdr + ':' + stack.position());
            crgn_.subIndex(src, register, stack);
        }
    }
