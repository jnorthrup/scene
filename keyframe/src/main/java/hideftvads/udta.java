 ...
 * <tr><td>Atoms</td><td>0x0</td><td>0x8</td><td></td><td>byte[]</td><td>{@link udtaVisitor#Atoms(ByteBuffer, int[], IntBuffer)}</td></tr>
 *
 * @see hideftvads.udta#Atom
 * @see hideftvads.udta#Atoms
 *      </table>
 */
public enum udta {
    Atom(0x8) {{
    ...
    static void index
        ...
        for (udta udta_ : values()) {
            String hdr = udta_.name();
            System.err.println("hdr:pos " + hdr + ':' + stack.position());
            udta_.subIndex(src, register, stack);
        }
    }
